...
class Hongbao extends ApiBase
{
    ...
    public function red_envelope_list(){
        ...
        $distance = 1;
        $red_number = 6;
        $red_list_sql = "SELECT id,order_id,lng,lat,state FROM think_red_order_info as i LEFT JOIN (SELECT id as not_id FROM think_red_order_info WHERE member_id = $user_id) as s ON s.not_id = i.order_id WHERE citycode = $citycode AND state = 0 AND s.not_id is null AND sqrt( ( (($lng-lng)*PI()*12656*cos((($lat+lat)/2)*PI()/180)/180) * (($lng-lng)*PI()*12656*cos ((($lat+lat)/2)*PI()/180)/180) ) + ( (($lat-lat)*PI()*12656/180) * (($lat-lat)*PI()*12656/180) ) )< $distance group by order_id limit $red_number;";
